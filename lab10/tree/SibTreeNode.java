...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
	  if (isValidNode()) {
		  SibTreeNode node = this.firstChild;
		  SibTreeNode kid = new SibTreeNode(myTree, item);
		  ...
		  if (c <= 1) {
			if (this.children() == 0) {
				...
				this.firstChild = kid;
				this.firstChild.nextSibling = temp;
				kid.parent = this;
			}
			myTree.size++;
		  }
		  ...
		  if (c > this.children()) {
			  ...
			  SibTreeNode pre = new SibTreeNode();
			  while (node != null && c > 1) {
				  pre = node;
				  node = node.nextSibling;
				  c--;
	  ...
	  if (isValidNode()) {
		  if (this.children() == 0) {
			  if (this.equals(myTree.root)) {
			      ...
			      myTree.root = null;
				  this.valid = false;
			  } else if (this.equals(this.parent.firstChild)) {
				  ...
				  if (count == this.parent.children()) {
					  ...
					  pre.nextSibling = this.nextSibling;
					  myTree.size--;
					  this.valid = false;
				  }
			  }

...
public class PixImage {
  ...
  public String toString() {
    // Replace the following line with your solution.
	  ...
	  for(int j = 0; j < height; j++) {
		  for(int i = 0; i < width; i++) {
		  result = result + "("+R[i][j]+","+G[i][j]+","+B[i][j]+")";
		  if(i == (width - 1)) {
			  result = result + "\n";
		  }
		  }
    ...
    // Replace the following line with your solution.
	  if(numIterations < 1) return this;
		  ...
		  for(int a = 1; a <= numIterations; a++) {
			  temp1 = new PixImage(width,height);
			  for(int j = 0; j < height; j++) {
				  for(int i = 0; i < width; i++) {
					  if(0 == i && 0 == j) {
						  final_R = (temp.R[i][j]+temp.R[i+1][j]+temp.R[i][j+1]+temp.R[i+1][j+1])/4;
						  final_G = (temp.G[i][j]+temp.G[i+1][j]+temp.G[i][j+1]+temp.G[i+1][j+1])/4;
						  final_B = (temp.B[i][j]+temp.B[i+1][j]+temp.B[i][j+1]+temp.B[i+1][j+1])/4;
					  } else if(i == (width - 1) && 0 == j) {
						  final_R = (temp.R[i][j]+temp.R[i-1][j]+temp.R[i-1][j+1]+temp.R[i][j+1])/4;
						  final_G = (temp.G[i][j]+temp.G[i-1][j]+temp.G[i-1][j+1]+temp.G[i][j+1])/4;
						  final_B = (temp.B[i][j]+temp.B[i-1][j]+temp.B[i-1][j+1]+temp.B[i][j+1])/4;
					  } else if(i == (width - 1) && j == (height - 1)) {
						  final_R = (temp.R[i][j]+temp.R[i][j-1]+temp.R[i-1][j-1]+temp.R[i-1][j])/4;
						  final_G = (temp.G[i][j]+temp.G[i][j-1]+temp.G[i-1][j-1]+temp.G[i-1][j])/4;
						  final_B = (temp.B[i][j]+temp.B[i][j-1]+temp.B[i-1][j-1]+temp.B[i-1][j])/4;
					  } else if(0 == i && j == (height - 1)) {
						  final_R = (temp.R[i][j]+temp.R[i][j-1]+temp.R[i+1][j-1]+temp.R[i+1][j])/4;
						  final_G = (temp.G[i][j]+temp.G[i][j-1]+temp.G[i+1][j-1]+temp.G[i+1][j])/4;
						  final_B = (temp.B[i][j]+temp.B[i][j-1]+temp.B[i+1][j-1]+temp.B[i+1][j])/4;
					  } else if(0 == j) {
						  final_R = (temp.R[i][j]+temp.R[i+1][j]+temp.R[i+1][j+1]+temp.R[i][j+1]+temp.R[i-1][j+1]+temp.R[i-1][j])/6;
						  final_G = (temp.G[i][j]+temp.G[i+1][j]+temp.G[i+1][j+1]+temp.G[i][j+1]+temp.G[i-1][j+1]+temp.G[i-1][j])/6;
						  final_B = (temp.B[i][j]+temp.B[i+1][j]+temp.B[i+1][j+1]+temp.B[i][j+1]+temp.B[i-1][j+1]+temp.B[i-1][j])/6;
					  } else if(i == (width - 1)) {
						  final_R = (temp.R[i][j]+temp.R[i][j+1]+temp.R[i-1][j+1]+temp.R[i-1][j]+temp.R[i-1][j-1]+temp.R[i][j-1])/6;
						  final_G = (temp.G[i][j]+temp.G[i][j+1]+temp.G[i-1][j+1]+temp.G[i-1][j]+temp.G[i-1][j-1]+temp.G[i][j-1])/6;
						  final_B = (temp.B[i][j]+temp.B[i][j+1]+temp.B[i-1][j+1]+temp.B[i-1][j]+temp.B[i-1][j-1]+temp.B[i][j-1])/6;
					  } else if(j == (height - 1)) {
						  final_R = (temp.R[i][j]+temp.R[i-1][j]+temp.R[i-1][j-1]+temp.R[i][j-1]+temp.R[i+1][j-1]+temp.R[i+1][j])/6;
						  final_G = (temp.G[i][j]+temp.G[i-1][j]+temp.G[i-1][j-1]+temp.G[i][j-1]+temp.G[i+1][j-1]+temp.G[i+1][j])/6;
						  final_B = (temp.B[i][j]+temp.B[i-1][j]+temp.B[i-1][j-1]+temp.B[i][j-1]+temp.B[i+1][j-1]+temp.B[i+1][j])/6;
						  ...
						  final_R = (temp.R[i][j]+temp.R[i+1][j]+temp.R[i+1][j+1]+temp.R[i][j+1]+temp.R[i-1][j+1]+temp.R[i-1][j]+temp.R[i-1][j-1]+temp.R[i][j-1]+temp.R[i+1][j-1])/9;
						  final_G = (temp.G[i][j]+temp.G[i+1][j]+temp.G[i+1][j+1]+temp.G[i][j+1]+temp.G[i-1][j+1]+temp.G[i-1][j]+temp.G[i-1][j-1]+temp.G[i][j-1]+temp.G[i+1][j-1])/9;
						  final_B = (temp.B[i][j]+temp.B[i+1][j]+temp.B[i+1][j+1]+temp.B[i][j+1]+temp.B[i-1][j+1]+temp.B[i-1][j]+temp.B[i-1][j-1]+temp.B[i][j-1]+temp.B[i+1][j-1])/9;
					  }
					  temp1.setPixel(i, j, (short)final_R, (short)final_G, (short)final_B);
				  }
			  }
			  temp = temp1;
		  }
